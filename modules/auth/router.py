...
@router.post(
    "/identify", response_model=IdentifyResponseSchema, response_model_exclude_none=True
)
async def identify(
    body: IdentifySchema, service: AuthService = Depends(get_auth_service)
):
...
@router.post(
    "/verify-code",
    response_model=VerifyCodeResponseSchema,
    response_model_exclude_none=True,
)
async def verify_code(
    body: VerifyCodeSchema, service: AuthService = Depends(get_auth_service)
):
    result = await service.verify_code(body.email, body.code)
    if not result:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid code or email"
        )
    return result
...
async def setup(
    body: SetupProfileSchema, service: AuthService = Depends(get_auth_service)
):
    result = await service.setup(
        body.setup_token, body.first_name, body.last_name, body.avatar_url
    )
    if not result:
        raise HTTPException(
...
async def refresh_token(
    body: RefreshTokenSchema, service: AuthService = Depends(get_auth_service)
):
