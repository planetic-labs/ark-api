...
async def create_user(
    ...
    role_result = await session.execute(
        select(Role).where(sa.func.lower(Role.name) == role_name)
    )
    ...
    if body.roles is not None:
        ...
        for r_name in body.roles:
            role_res = await session.execute(
                select(Role).where(sa.func.lower(Role.name) == r_name.lower())
            )
    ...
    if body.personal_permissions is not None:
        ...
        for p_key in body.personal_permissions:
            perm_res = await session.execute(
                select(Permission).where(Permission.key == p_key)
            )
...
async def create_role(
    ...
    existing = await session.execute(
        select(Role).where(sa.func.lower(Role.name) == body.name.lower())
    )
    if existing.scalar_one_or_none():
