...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_service_clients_token_hash"),
        "service_clients",
        ["token_hash"],
        unique=True,
    )
    ...
    op.add_column(
        "refresh_tokens", sa.Column("device_info", sa.String(length=512), nullable=True)
    )
    ...
    op.add_column(
        "users",
        sa.Column(
            "status", sa.String(length=50), nullable=False, server_default="created"
        ),
    )
    op.add_column(
        "users",
...
def downgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column(
            "role",
            postgresql.ENUM("MASTER", "WARRIOR", "STUDENT", "ADMIN", name="userrole"),
            autoincrement=False,
            nullable=False,
        ),
    )
