...
# Protected release flow: branch -> PR -> checks -> merge -> release -> Docker publish.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
PYPROJECT_PATH="$PROJECT_ROOT/pyproject.toml"
REPOSITORY="${REPOSITORY:-planetic-labs/ark-api}"
WORKFLOW="${WORKFLOW:-build-docker.yml}"
...
[[ -z "$(git status --porcelain)" ]] || {
    echo "The working tree is not clean. Commit or stash changes before creating a release." >&2
    exit 1
}
...
if [[ "$CURRENT_VERSION" == "$VERSION_NUM" ]]; then
    ...
    PR_URL="$(gh pr create --repo "$REPOSITORY" --base main --head "$RELEASE_BRANCH" \
        --title "chore: bump version to $VERSION_NUM" --body "Prepare release $VERSION.")"
    PR_NUMBER="${PR_URL##*/}"
    ...
    CHECK_COUNT=0
    for _ in {1..12}; do
        CHECK_COUNT="$(gh pr view "$PR_NUMBER" --repo "$REPOSITORY" \
    ...
    gh pr checks "$PR_NUMBER" --repo "$REPOSITORY" --watch >/dev/null || {
...
if gh release view "$VERSION" --repo "$REPOSITORY" >/dev/null 2>&1; then
    ...
    RELEASE_URL="$(gh release create "$VERSION" --repo "$REPOSITORY" --target main \
...
RUN_ID=""
for _ in {1..12}; do
    RUN_ID="$(gh run list --repo "$REPOSITORY" --workflow "$WORKFLOW" --event release \
...
echo "Release $VERSION and its Docker images were published successfully."
...
mapfile -t OLD_RUNS < <(
    gh run list --repo "$REPOSITORY" --limit 1000 --created "<$TODAY_DATE" \
